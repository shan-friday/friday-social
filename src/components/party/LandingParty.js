...
const LandingParty = (props) => {
    ...
    useEffect(() => {
        ...
        if (!activity.isExtensionInstalled)
            props.history.push('/installExtension');

        if (partyId && regex.test(partyId)) {
            // dispatch(joinSession(auth.user, partyId));
            chrome.runtime.sendMessage('jjpkejikllkphemiloeipdmbaonlpfnm', { action: 'open_app_external', message: 'open_app_external', partyId });
            props.history.push('/');
        } else
            props.history.push('/');
    })
